                ...
                @foreach(\App\Models\ProductIntroduction::latest()->get() as $intro)
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-sm font-medium text-gray-900">{{ $intro->title }}</span>
                            @if($intro->is_active)
                        ...
                        <div class="text-sm text-gray-500 mt-1">Dibuat: {{ $intro->created_at->format('d M Y') }}</div>

                        <div class="grid grid-cols-2 gap-2 mt-3">
                            <a href="{{ route('admin.product-introduction.edit', $intro->id) }}" class="inline-flex items-center justify-center px-3 py-2 rounded-md text-xs font-semibold bg-pink-50 text-pink-700 border border-pink-100 hover:bg-pink-100">
                                Edit
                            </a>
                            @if(!$intro->is_active)
                                <form action="{{ route('admin.product-introduction.set-active', $intro->id) }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('admin.product-introduction.destroy', $intro->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengenalan produk ini?')" class="col-span-2">
                                    @csrf
                                    @method('DELETE')
