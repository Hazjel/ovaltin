...
<div class="space-y-6 max-w-3xl mx-auto">
    ...
    <div class="bg-white shadow rounded-xl p-6 border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Status Notifikasi</h3>
        ...
        @if(config('services.whatsapp.qr_secret'))
            <div class="mt-4 pt-4 border-t border-gray-100">
                <a href="#"
                   onclick="window.open('{{ rtrim(config('services.whatsapp.url'), '/') }}/qr-page?token={{ config('services.whatsapp.qr_secret') }}&t=' + Date.now(), '_blank'); return false;"
                   class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h5M20 20v-5h-5M4 9a8 8 0 0114-5.3M20 15a8 8 0 01-14 5.3" />
                    </svg>
                    Reconnect WhatsApp
                </a>
    ...
    {{-- Form Pengaturan --}}
    <div class="bg-white shadow rounded-xl p-6 border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Konfigurasi</h3>
        ...
        <form action="{{ route('admin.notification-settings.update', $settings->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input
                    ...
                    class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-pink-500 focus:border-pink-500 sm:text-sm"
                >
                <p class="mt-1 text-xs text-gray-500">Format: 62xxx (tanpa + atau 0 di depan). Contoh: 6281234567890</p>
            ...
            <div>
                ...
                <textarea
                    ...
                    class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-pink-500 focus:border-pink-500 sm:text-sm font-mono text-sm"
                ...
                <div class="mt-2 bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
                    ...
                    <ul class="space-y-0.5">
                        ...
                        <li><code class="bg-gray-200 px-1 rounded">{emoji}</code> — Emoji waktu</li>
            ...
            <div class="flex justify-end pt-2">
                <button type="submit"
                    class="inline-flex items-center px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition">
                    Simpan Pengaturan
                </button>
            </div>
        </form>
    ...
    {{-- Test Send --}}
    <div class="bg-white shadow rounded-xl p-6 border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Kirim Pesan Test</h3>
        ...
        <form action="{{ route('admin.notification-settings.test') }}" method="POST">
            @csrf
            <button type="submit"
                class="inline-flex items-center px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-gray-700 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition"
    ...
    <div class="bg-white shadow rounded-xl p-6 border border-gray-100">
        ...
        @if($logs->isEmpty())
            <div class="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-200 text-sm">
    ...
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-5">
        <h4 class="font-semibold text-gray-800 mb-3">Cara Kerja Notifikasi</h4>
        <ul class="text-sm text-gray-600 space-y-2 list-disc list-inside">
            <li><strong>Jadwal:</strong> notifikasi dikirim setiap {{ implode(', ', $selectedDays) }}</li>
            ...
            <li>Berhenti otomatis jika admin klik "Tidak ada penjualan" di popup saat membuka halaman Data Penjualan</li>
            <li>Berhenti otomatis jika data penjualan sudah diisi minggu ini</li>
            <li>Dijalankan otomatis lewat Laravel scheduler (cron job server)</li>
        </ul>
    </div>
